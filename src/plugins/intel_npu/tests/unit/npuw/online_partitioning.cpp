...
::intel_npu::Config createConfigWithKeepBlockSize(std::size_t size) {
    ...
    std::map<std::string_view, std::string_view> cfg_map = {{"NPUW_ONLINE_KEEP_BLOCK_SIZE", std::to_string(size)}};
    cfg.update(cfg_map);
    return cfg;
}
...
TEST(OnlinePartitioningTest, Partitioning_Avoids_Pipeline_None) {
    ...
    auto cfg = ::intel_npu::Config(opt_desc);
    ::intel_npu::registerNPUWOptions(*opt_desc);
    std::map<std::string_view, std::string_view> cfg_map = {{"NPUW_ONLINE_AVOID", "Op:Sin/NPU,Op:Cos/NPU"},
